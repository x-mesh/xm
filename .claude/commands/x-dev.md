...
Local install, validate, update, and test x-core plugins without pushing to remote.
...
```
REPO_ROOT = /Users/jinwoo/work/project/agentic/x-core
MARKETPLACE_NAME = x-core
PLUGINS = [x-agent, x-build, x-op, x-core]
```
...
Show current state of x-core plugins.
...
Look for marketplace with name containing "x-core" or source path matching REPO_ROOT.
...
claude plugin validate REPO_ROOT/x-core 2>&1
```
...
🔧 x-dev Status
  ...
  Marketplace:
    x-core  ✅ registered (local: /Users/jinwoo/work/project/agentic/x-core)
    — or —
    x-core  ❌ not registered (run: /x-dev marketplace add)

  Installed plugins:
    x-agent   ✅ v1.0.0 (scope: user)
    x-build   ✅ v1.0.0 (scope: user)
    x-op      ❌ not installed
    x-core     ❌ not installed
...
Output:
```
✅ Marketplace "x-core" registered from local path
   Source: REPO_ROOT
...
```
ℹ️ Marketplace "x-core" is already registered.
```
...
```bash
claude plugin marketplace remove x-core
```

Output:
```
✅ Marketplace "x-core" removed.
```
...
Check `claude plugin marketplace list --json`. If x-core marketplace is not found:
```
...
From `$ARGUMENTS`, extract the plugin name after "install". Accept both `x-op` and `x-core@x-op` format.

If plugin name is not in PLUGINS list:
```
❌ Unknown plugin: {name}
   Available: x-agent, x-build, x-op, x-core
```

### Step 3: Install

```bash
claude plugin install x-{name}@x-core -s user
```
...
```bash
claude plugin install x-agent@x-core -s user
claude plugin install x-build@x-core -s user
claude plugin install x-op@x-core -s user
claude plugin install x-core@x-core -s user
```
...
```bash
claude plugin uninstall x-{name}@x-core -s user
```
...
```bash
claude plugin update x-{name}@x-core -s user
```
...
claude plugin validate REPO_ROOT/x-core
```
...
```
This will uninstall all x-core plugins and remove the local marketplace. Continue? (y/n)
```

### Step 2: Uninstall all

```bash
claude plugin uninstall x-agent@x-core -s user 2>&1 || true
claude plugin uninstall x-build@x-core -s user 2>&1 || true
claude plugin uninstall x-op@x-core -s user 2>&1 || true
claude plugin uninstall x-core@x-core -s user 2>&1 || true
claude plugin marketplace remove x-core 2>&1 || true
```
...
🧹 Reset complete
   ...
   Uninstalled: x-agent, x-build, x-op, x-core
   Marketplace: x-core removed
