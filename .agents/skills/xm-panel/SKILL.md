---
name: xm-panel
description: "Cross-vendor entry point + adversarial panel engine. `/xm:panel <verb>` routes multi-model work to the matching consumer in --cross-vendor mode (review→x-review, plan(brainstorm)/debate/council→x-op, solve→x-solver, eval→x-eval, consensu..."
---
...
>
> **Fallback** (only when `xm` is not in PATH — rare; `.codex/xm` is NOT exported to Bash subprocesses):
> ```bash
...
- `$ARGUMENTS` means the user text following the current Skill mention (`$xm-panel` or `$xm:panel`). Treat it as input context, not as a literal shell environment variable unless a quoted command explicitly requires one.
