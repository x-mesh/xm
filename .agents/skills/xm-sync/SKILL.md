...
- `$ARGUMENTS` means the user text following the current Skill mention (`$xm-sync` or `$xm:sync`). Treat it as input context, not as a literal shell environment variable unless a quoted command explicitly requires one.
