---
name: xm-wt
description: "Session worktree — isolate the whole current session in a git worktree, then land it back to the parent branch. Use for \"worktree로 작업\", \"이 세션을 worktree에서\", \"격리해서 작업하고 부모로 머지\", \"/xm:wt\", or when the user wants the ENTIRE session (not one..."
---
...
- `$ARGUMENTS` means the user text following the current Skill mention (`$xm-wt` or `$xm:wt`). Treat it as input context, not as a literal shell environment variable unless a quoted command explicitly requires one.
