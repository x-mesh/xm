---
name: xm-inbox
description: "Receiving side of /xm:toss — list, take, resolve, or drop cross-project bug reports that landed in this project's .xm/inbox/. Use when the user asks to check the inbox, \"받은 편지함 봐줘\", \"what did other projects toss at us\", or wants to start..."
---
...
- `$ARGUMENTS` means the user text following the current Skill mention (`$xm-inbox` or `$xm:inbox`). Treat it as input context, not as a literal shell environment variable unless a quoted command explicitly requires one.
