---
name: recall
description: "Cross-session artifact index. Find and read outputs from previous sessions — reviews, op strategy results, plans/PRDs, eval scores, probe verdicts — stored under .xm/. Use when the user asks to recall/find/look up a past review/op/plan,..."
---
...
- `$ARGUMENTS` means the user text following the current Skill mention (`$xm-recall` or `$xm:recall`). Treat it as input context, not as a literal shell environment variable unless a quoted command explicitly requires one.
