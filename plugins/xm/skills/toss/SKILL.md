---
name: toss
description: "File a reproducible bug report directly into another registered x-kit project's inbox — captures a repro command + output (secret-redacted), a fix direction, writes a durable local outbox record, and best-effort notifies the target via m..."
---
...
>
> **Fallback** (only when `xm` is not in PATH — rare; `.codex/xm` is NOT exported to Bash subprocesses, so don't rely on it bare):
> ```bash
...
- `$ARGUMENTS` means the user text following the current Skill mention (`$xm-toss` or `$xm:toss`). Treat it as input context, not as a literal shell environment variable unless a quoted command explicitly requires one.
