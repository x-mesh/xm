...
- `attention [--json] [--budget N] [--since 30d] [--backfill] [--git] [--max-commits N] [--dry-run]` — collect and rank gate escapes, contested findings, surviving mutants, and revived dismissals without changing gate decisions (`--budget 0` = all)
- `attention --backfill --git [--since 30d] [--max-commits N]` — also mine git history for shipped defects: `fix`/`revert` commits become escape rows tagged `source:git`, `escape_class:shipped_defect`, with `fix_shipped_test` recording whether the fix also shipped a test. `perf` is classified but never counted as a defect. Works on any repo with history, no panel run required. Optional per-repo tuning in `.xm/attention-git.json`; `.xm-review.json` `generated_copy_roots` are excluded automatically
- `attention --ack <item-id> [--note <text>]` — acknowledge an attention item so it is excluded from subsequent queues
- `verify-tests [--since 30d] [--limit N] [--timeout-ms M] [--json]` — L3 check on the `fix_shipped_test` claim above. Replays each sampled fix commit's tests against its PARENT in a throwaway worktree: a test that detects its own bug fails there, one that passes never exercised the defect. Separates `catches` (an assertion failed) from `catches_by_binding` (only a load error — the test names new API but detects nothing), so the corrected rate does not inherit false confidence. Read-only for the primary checkout; bun projects only. Report: `.xm/review/escape-verify.json`
- `mutate [--list] [--json]` — list tasks whose linked worktree has changed files in a supported language; read-only, and no arguments prints the same list
- `mutate --project <name> --task <id> [--base <ref>] [--lang <names>] [--timeout-ms M] [--json]` — mutation-test the task's changed lines with the external tool for each language (cargo-mutants, StrykerJS, gomutants, Muter) and append survivors to the attention queue. `xm mutate --diff <base>` runs the same check without a task
