...
export const SCHEMA = [
  ...
  },
  {
    ...
    scope: 'local',
    default: false,
    description: 'Agent PreToolUse에서 budget hard-cap을 실제 차단할지 여부. 기본 false=warn-only',
  },
  {
    key: 'budget.fallback',
    group: 'budget',
    ...
    enum: ['downgrade'],
    default: null,
    description: 'hard-cap 예측 시 자동 model route만 opus→sonnet→haiku로 낮춤. 명시적 model pin은 변경하지 않음',
  },
  {
    key: 'budget.prediction_max_age_hours',
    group: 'budget',
    type: 'number',
    nullable: true,
    scope: 'local',
  ...
  {
    key: 'budget.emergency_reserve_usd',
    group: 'budget',
    type: 'number',
    nullable: true,
    scope: 'local',
    default: null,
    description: 'x-ship/release 경로만 사용할 수 있는 hard-cap 외 별도 emergency reserve(USD)',
  },
  {
    key: 'memmesh.mirror',
    group: 'misc',
  ...
  {
    key: 'cache.store_content',
    group: 'misc',
    type: 'boolean',
    scope: 'local',
    default: false,
    description: '캐시에 원문 content 저장 허용. 기본 false는 content hash와 byte 수만 저장',
  },
  {
    key: 'cache.ttl_days',
    group: 'misc',
    ...
    default: 7,
    min: 0.000001,
    description: '캐시 조회·정리 만료 기간(일). 기본 7일',
  },
  {
