...
function targetSet(specs,cwd){const bounds=[];try{for(const spec of specs)bounds.push(openBoundTarget(cwd,spec.path||spec.target,{expectedIdentity:spec.originalIdentity||spec.expectedIdentity,expectedBytes:spec.originalBytes||spec.expectedBytes,expectedMode:spec.originalMode??spec.expectedMode}));return bounds;}catch(error){try{closeBoundTargets(bounds);}catch{}throw error;}}
async function runBoundMutate({target,targets=null,command,timeoutMs=30_000,cwd=process.cwd(),signal=null,replacementBytes=null,expectedIdentity=null,expectedBytes=null,expectedMode=null}) {
  const specs=targets||[{path:target,originalIdentity:expectedIdentity,originalBytes:expectedBytes,originalMode:expectedMode}],bounds=targetSet(specs,cwd),activeIndex=Math.max(0,specs.findIndex(spec=>resolve(cwd,spec.path||spec.target)===resolve(cwd,target))),active=bounds[activeIndex];let child,timedOut=false,installed=bounds.map(bound=>bound.bytes);
  try {
    if(replacementBytes){replaceBytes(active.fd,replacementBytes);fchmodSync(active.fd,active.mode);assertTargetIdentity(active.path,active.identity);installed[activeIndex]=replacementBytes;}
