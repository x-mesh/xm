...
async function runBoundMutate({target,targets=null,command,timeoutMs=30_000,cwd=process.cwd(),signal=null,replacementBytes=null,expectedIdentity=null,expectedBytes=null,expectedMode=null}) {
  ...
  try {
    if(replacementBytes){installBytes(active.fd,replacementBytes,active.bytes);installed[activeIndex]=replacementBytes;fchmodSync(active.fd,active.mode);assertTargetIdentity(active.path,active.identity);}
