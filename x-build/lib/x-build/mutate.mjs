...
async function runBoundMutate({target,targets=null,command,timeoutMs=30_000,cwd=process.cwd(),signal=null,replacementBytes=null,expectedIdentity=null,expectedBytes=null,expectedMode=null}) {
  ...
  try {
    ...
    let restoreError=null;const restorable=[];
    try{for(let index=0;index<bounds.length;index+=1){const bound=bounds[index],current=readAll(bound.fd),mode=Number(fstatSync(bound.fd,{bigint:true}).mode&0o777n);if(current.equals(installed[index])&&mode===bound.mode)restorable.push(bound);else restoreError??=Object.assign(new Error('mutate target changed concurrently during test; conflicting bytes were preserved'),{exitCode:2});try{assertTargetIdentity(bound.path,bound.identity);}catch(error){restoreError??=error;}}for(const bound of restorable){replaceBytes(bound.fd,bound.bytes);fchmodSync(bound.fd,bound.mode);}for(const bound of restorable){const after=readAll(bound.fd),mode=Number(fstatSync(bound.fd,{bigint:true}).mode&0o777n);if(!after.equals(bound.bytes)||mode!==bound.mode)fail('mutate restore failed; recover target from snapshot');}}catch(error){restoreError??=error;}
