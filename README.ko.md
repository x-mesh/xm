...
### 다중 도구 설치 (Cursor / Codex / Kiro / Antigravity)
...
# 사용자 글로벌 설치 (~/.cursor/, ~/.codex/, ~/.kiro/, ~/.gemini/)
...
- 설치 시 `<install-root>/.<tool>/xm/manifest.json`에 SHA-256 + HMAC self-checksum 기록. `--verify`는 해시 재계산, `--uninstall`은 기록된 파일만 정확히 제거.
