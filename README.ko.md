...
<p align="center">
  <a href="https://github.com/x-mesh/xm/releases"><img src="https://img.shields.io/badge/version-2.4.52-blue" alt="Version" /></a>
...
크로스 모델 적대 리뷰 패널. 여러 모델 CLI(claude/codex/agy/cursor)를 같은 타깃에 돌려 서로의 finding을 반박하게 하고, **합의**(N/M 동의 — 신뢰도)와 **다양성**(한 모델만 잡은 것)을 분리한 verdict를 종합합니다. 오케스트레이터가 도구 중립 CLI라 리더가 고정 모델이 아닙니다.
...
| 4 | 게이트 | 페이즈 종료 게이트 5개 (`research/plan/execute/verify/close-exit`) — `auto` / `human-verify` / `quality` |
| 5 | worktree | 병렬 worktree 키를 3-tier 스코프(build-local > shared > global)로 편집 + `gate_policy` severity 목록 |
| 6 | 기타 | `mode` · `drift.drift_threshold` · `scan_roots` · `pipelines` |
...
**Codex 지원.** `xm install --target codex`는 역할별 에이전트 config(`<.codex>/xm/agents/xm-{planner,executor,reviewer}.config.toml`)와 프로필별 config(`<.codex>/xm-{economy,default,max}.config.toml`)를 생성하고, multi-agent 기능을 게이트하며, x-build 프롬프트에 Codex Orchestration Overlay를 추가합니다. `xm build run --json`은 태스크마다 additive `model_vendor` / `model_by_vendor` 필드를 내보내되 `task.model`은 그대로 둡니다. Codex 어댑터는 effort를 `-c model_reasoning_effort=<level>`로 전달하며, `resume` 시 exec 플래그가 `resume` 서브커맨드보다 앞에 옵니다.
