...
| Codex CLI | `plugins/xm/.codex-plugin/plugin.json` + `plugins/xm/skills/<skill>/SKILL.md` + `.agents/plugins/marketplace.json` | `codex plugin add xm@<marketplace>` 후 `$xm:<skill>` | `.codex/hooks.json` / `~/.codex/hooks.json` (`codex features enable hooks` 또는 `[features] hooks=true` 필요) |
