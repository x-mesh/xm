...
```
/xm:kit:init              # trace-session 훅을 ~/.claude/에 설치
/xm:kit:init status       # 설치 상태 확인
/xm:kit:init uninstall    # 훅 파일 + settings.json 항목 제거
/xm:kit:init --no-hooks   # CLI만 설치 (현재는 no-op, 예약됨)
```
...
인스톨러는 `~/.local/bin/xm:kit`을 설치하고(`XM_BIN_DIR`로 경로 변경 가능, `PATH`에 있어야 함), `claude` CLI가 `PATH`에 있으면 `marketplace.json`의 모든 플러그인(x-build, x-agent, x-op, x-solver, x-review, x-trace, x-memory, x-eval, x-probe, x-humble, x-dashboard, xm)을 `claude plugin install <p>@xm -s user`로 일괄 설치합니다. 설치 후 Claude Code에서 `/reload-plugins`로 활성화하세요. `claude`가 `PATH`에 없으면 CLI 래퍼만 설치되고 수동 설치용 플러그인 목록이 출력됩니다.

#### 전역 훅 설치 (`xm init`)

bash `xm init` 서브커맨드는 `/xm:kit:init` 슬래시 커맨드와 동일한 동작을 합니다 — Skill 트레이싱 훅을 **사용자 스코프**(`~/.claude/`)에 설치합니다 (머신당 1회):
...
`~/.claude/hooks/xm-trace-session.mjs`를 복사하고 `~/.claude/settings.json`의 `PreToolUse`/`PostToolUse`에 Skill matcher를 병합합니다. 기존 훅(mem-mesh 등)은 보존되며, 수정 시 타임스탬프 백업이 생성됩니다. Claude Code 밖에서 실행해야 할 때는 bash 경로를 쓰고, 그 외에는 `/xm:kit:init`을 권장합니다.
...
xm은 20년 엔지니어링 경험에서 추출한 이 판단 패턴을 모든 에이전트 프롬프트에 직접 심습니다. 결과: 리스트를 패턴 매칭하는 것이 아니라, 맥락을 파악하며 추론하는 에이전트.
...
| xm | 번들 + 설정 + 파이프라인 | `/xm:kit pipeline release` |
...
릴리스 자동화 — 커밋 스쿼시, 버전 범프, 푸시. xm 마켓플레이스 플러그인과 독립 프로젝트(Node.js, Rust, Python, Go) 모두 지원.
...
xm은 플러그인 간 사고 원칙을 순환 피드백으로 연결합니다.
...
xm에는 코어와 도메인 영역을 아우르는 37개 전문가 에이전트가 포함되어 있습니다. 플러그인이 자동으로 사용합니다 (예: x-op refine이 주제별 전문가 주입; x-review는 `--specialists`로 사용).

```bash
/xm:kit agents list                        # 37개 전문가 목록
/xm:kit agents match "결제 API 설계"       # 주제에 맞는 에이전트 찾기
/xm:kit agents get security --slim         # 전문가 규칙 보기
```
...
```bash
/xm:kit config set agent_max_count 10              # 병렬 에이전트 10개
/xm:kit config set team_default_leader_model opus  # Team Leader 모델
/xm:kit config set team_max_members 5              # 팀당 최대 멤버
/xm:kit config show
```
...
```bash
/xm:kit config set model_profile economy           # Sonnet 중심, 최대 절약
/xm:kit config set model_profile default           # 기본값 — Opus 중심 (Opus 4.7 기준)
/xm:kit config set model_profile max               # 전부 Opus, 품질 최우선
/xm:kit config set budget '{"max_usd": 5.0}'       # 세션 예산 한도 설정
```
...
역할별 오버라이드: `/xm:kit config set model_overrides '{"architect": "opus"}'`로 프로필 위에 개별 설정 가능.
...
> **핵심:** haiku는 동작하는 코드를 만들지만 엣지케이스가 부족합니다. sonnet은 대부분의 작업에 프로덕션급이고, opus는 훨씬 높은 비용으로 방어적 견고함을 추가합니다. 프로필로 트레이드오프를 선택하세요: `economy`(Sonnet 중심) vs `default`(Opus 중심) vs `max`(전부 Opus). 워크로드별 추정치는 `/xm:build forecast`.

#### 자동 모델 라우팅

xm은 명령을 가장 저렴한 적합 모델로 자동 라우팅합니다. 조회/표시 명령은 **haiku** (~78% 저렴)를, 추론 작업은 sonnet 또는 opus를 사용합니다.
