...
Full probe workflow — premise extraction, evidence gathering, verdict deliberation. Invoked by x-probe main flow.

## Applies to
Called by x-probe routing when `$ARGUMENTS` matches probe intent or explicit `session probe`.
...
1) Yes → /xm:build init "{idea}" (premises + verdict auto-injected into CONTEXT.md)
...
1) 네 → /xm:build init "{idea}" (가정과 결론이 자동으로 반영됩니다)
2) 아직요 — 더 조사가 필요합니다
```
...
1) Yes → /xm:humble review "x-probe: {idea} — killed because: {reason}"
2) No — move on
```
...
1) 네 → /xm:humble review "x-probe: {idea} — 중단 이유: {reason}"
2) 아니요 — 넘어가기
```
