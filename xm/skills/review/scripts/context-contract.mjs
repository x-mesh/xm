...
function cleanItems(value, field, { required = false } = {}) {
  if (!Array.isArray(value) || (required && value.length === 0)) {
    throw new Error(`review context ${field} must be ${required ? 'a non-empty' : 'an'} array`);
  }
  ...
  return value.map((item, index) => {
    ...
    if (Object.keys(item).some((key) => !['id', 'text'].includes(key))) {
      throw new Error(`review context ${field}[${index}] contains unknown fields`);
    }
    const id = cleanText(item.id, `${field}[${index}].id`, { max: 64 });
    if (!ID_RE.test(id)) throw new Error(`review context ${field}[${index}].id is invalid`);
    if (ids.has(id)) throw new Error(`review context ${field} contains duplicate id ${id}`);
    ids.add(id);
...
function cleanAcceptanceChecks(value) {
  ...
  return value.map((item, index) => {
    ...
    if (Object.keys(item).some((key) => !['id', 'description', 'command'].includes(key))) {
      throw new Error(`review context acceptance_checks[${index}] contains unknown fields`);
    }
...
export function normalizeReviewContext(value) {
  ...
  if (bytes > REVIEW_CONTEXT_MAX_BYTES) throw new Error(`review context exceeds ${REVIEW_CONTEXT_MAX_BYTES} bytes`);
  return normalized;
}
