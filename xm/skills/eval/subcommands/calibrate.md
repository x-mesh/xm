...
Validate LLM judge scores against human judgment. Surface systematic per-criterion bias so you know where to trust the judges and where to add guardrails. Usage: `/xm:eval calibrate --rubric <name> [--samples N]`.
...
## Applies to
Invoked via `/xm:eval calibrate ...`. Calibration data is consumed by `report` (surfaces calibration age and systematic criteria). See `references/storage-layout.md` for schema.
