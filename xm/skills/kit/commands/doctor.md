...
Unified diagnostic — check every piece of xm's install footprint in one pass.

## Commands

| Command | Description |
|---------|-------------|
| `xm doctor` | Report status of hooks, settings, x-sync, PATH, and Bun |
| `xm doctor --fix` | Automatically fix whatever is safe to fix (re-run init for missing hooks/settings; prompt before network install) |
...
| `⏭️` | Not applicable for this context (e.g., xm-repo-only hook in a user project; server-only check when server not installed) |
...
check('hooks/block-marketplace-copy.mjs', () => {
  ...
  // Heuristic requires BOTH a marketplace.json AND the xm skill file — the combination is unique to this repo
  ...
  const inXKitRepo = fs.existsSync(path.join(PROJECT, '.claude-plugin/marketplace.json')) &&
                     fs.existsSync(path.join(PROJECT, 'xm/skills/kit/SKILL.md'));
  ...
  if (!inXKitRepo) return { status: '⏭️', detail: 'xm-repo-only — expected to be absent in user projects' };
  if (!fs.existsSync(dst)) return { status: '❌', detail: 'missing in xm repo — copy from marketplace manually', fixable: false, note: 'cp ' + src + ' ' + dst };
  if (fs.existsSync(src) && fs.readFileSync(src,'utf8') !== fs.readFileSync(dst,'utf8'))
...
check('x-sync client', () => {
  try {
    ...
    return { status: '⚠️', detail: 'not installed — run xm init (network required)', networkFix: true };
  }
});
...
if (FIX) {
  ...
  if (localFixes.length > 0) {
    ...
    // Delegate to init (leader invokes 'xm init' separately — doctor only flags them)
    console.log('   → run: xm init');
  }
  if (networkFixes.length > 0) {
    console.log('\n🌐 Network install required for: ' + networkFixes.map(r => r.name).join(', '));
    console.log('   → run: xm init  (will curl x-sync install.sh)');
  }
  if (manualFixes.length > 0) {
    console.log('\n✋ Manual fixes required (xm init does not handle these):');
...
2. For **local fixes** (hooks out of date, settings missing entries): automatically run `xm init` — safe, no network
3. For **network fixes** (x-sync missing): use AskUserQuestion to confirm before running `xm init` (which does curl). Header: `x-sync install`, options: `Install now` / `Skip`
