...
Use the structured x-build JSON as the authoritative routing source. Consume \`model_routes.plan.model_by_vendor.codex\` for adaptive planned-route planning and \`model_routes.execute.model_by_vendor.codex\` for adaptive execution. Consume \`prd_writer.model_by_vendor.codex\` for legacy PRD generation, each \`agents_spec[*].model_by_vendor.codex\` for Research, each \`agents[*].model_by_vendor.codex\` for consensus, and each legacy Execute task's \`task.model_by_vendor.codex\`. These fields are authoritative for Codex; do not infer Codex routing from Claude-tier \`model\` alone.

For a planned route, split the exact plan spec into model and optional effort. Pass both values to the native planner subagent. If native spawning cannot accept that exact spec, use \`codex exec -c model=<model> -c model_reasoning_effort=<effort>\` with the same values. Run the complete x-plan Standard workflow inside the planner. Read its persisted envelope after it returns. If the planner call fails, the artifact is missing, or the envelope does not set \`executable=true\`, stop before Execute. You must not continue with a root-session draft or an unconfigured \`codex exec\` call. For Execute, spawn one Codex subagent per parallel-safe task with the configured execute spec.
