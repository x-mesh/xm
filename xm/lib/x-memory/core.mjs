...
export function modifyJSON(path, mutator) {
  ...
  for (let attempt = 0; attempt < 20; attempt++) {
    try {
      ...
      if (!staleCleared && attempt >= 2 && clearStaleLock(lockPath)) {
        staleCleared = true;
        console.error(`  ${C.yellow}⚠ Cleared stale lock on ${basename(path)} (previous holder no longer alive)${C.reset}`);
        continue;
      }
  ...
  // Fallback: proceed without lock — degraded mode, surface it loudly
  console.error(`  ${C.red}⚠ Lock contention on ${basename(path)} — proceeding without lock (data race possible)${C.reset}`);
  const data = readJSON(path);
  const result = mutator(data);
  const out = result !== undefined ? result : data;
  writeJSON(path, out);
