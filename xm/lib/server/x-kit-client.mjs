#!/usr/bin/env node
/**
 * xm-client.mjs — Thin CLI client for xm persistent server
 *
 * Usage: node xm-client.mjs <plugin> <args...>
 *   e.g. node xm-client.mjs x-build status
 *        node xm-client.mjs x-build tasks add "my task" --size medium
 *
 * Flow:
...
async function autoStart() {
  ...
  const serverPath = resolve(dirname(new URL(import.meta.url).pathname), 'xm-server.mjs');
...
async function handleServerCommand(args) {
  ...
  console.log('Usage: xm-client server <start|stop|status>');
}
