...
/**
 * Pair every measured `actual` with the prediction it was recorded against and
 * report MAPE. This is the terminal-side read of the ledger that
 * `appendPredictionLog`/`appendPredictionActual` write; without it the ledger is
 * write-only outside the dashboard and a CLI user cannot tell whether `forecast`
 * numbers are trustworthy.
 *
 * Rules deliberately match the dashboard's `readPredictionCalibration`:
 *   - an `event_id` is counted once (rotation means `.1` and the live file overlap)
 *   - `actual == 0` rows are kept in `pairs` for disclosure but excluded from
 *     MAPE, whose denominator would be undefined
 *   - a prediction with no matching actual is not an error, just uncalibrated
 *
 * @returns {{ pairs: Array<{predicted:number, actual:number, model:string|null, size:string|null, role:string|null, task_id:string|null, timestamp:string}>, mape: number|null, samples: number, excluded_zero_actual: number }}
 */
export function readPredictionAccuracy({ project = null } = {}) {
  const events = readCalibrationEvents();
  const seen = new Set();
  ...
  for (const event of events) {
    ...
    if (event.type === 'prediction' && finiteNonNegative(event.predicted_cost_usd)) {
      predictions.set(event.event_id, event);
    } else if (event.type === 'actual' && typeof event.prediction_event_id === 'string'
      && finiteNonNegative(event.actual_cost_usd)
  ...
  for (const actual of actuals) {
    ...
    if (project && (actual.project ?? prediction.project) !== project) continue;
    pairs.push({
      timestamp: actual.timestamp,
      predicted: prediction.predicted_cost_usd,
      actual: actual.actual_cost_usd,
      task_id: actual.task_id ?? prediction.task_id ?? null,
      model: prediction.model ?? null,
      size: prediction.size ?? null,
      role: prediction.role ?? null,
    });
  }

  const usable = pairs.filter((pair) => pair.actual > 0);
  const mape = usable.length
    ? Number((usable.reduce((sum, p) => sum + Math.abs(p.predicted - p.actual) / p.actual, 0) / usable.length * 100).toFixed(2))
    : null;
  return {
    pairs,
    mape,
    samples: usable.length,
    excluded_zero_actual: pairs.length - usable.length,
  };
}
