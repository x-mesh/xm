...
function listCmd(args) {
  ...
  if (items.length === 0) {
    process.stdout.write('No projects registered. Run `xm project import` or `xm project add` to populate.\n');
    return 0;
  }
...
function removeCmd(args) {
  const target = args[0];
  if (!target) {
    process.stderr.write('Usage: xm project remove <id|path>\n');
    return 2;
  }
...
function archiveCmd(args, archived) {
  const target = args[0];
  if (!target) {
    process.stderr.write(`Usage: xm project ${archived ? 'archive' : 'unarchive'} <id|path>\n`);
    return 2;
  }
...
function registerCmd(args) {
  ...
  if (!quiet && result.action === 'added') {
    process.stderr.write(`[xm] auto-registered project: ${result.entry.id} (${result.entry.path})\n`);
  }
  return 0;
}

function helpCmd() {
  process.stdout.write(`xm project — manage the project registry at ${REGISTRY_PATH}

Usage:
  xm project list [--json] [--archived]
  xm project add [<path>] [--id <id>] [--name <name>]
  xm project remove <id|path>
  xm project archive <id|path>
  xm project unarchive <id|path>
  xm project import [<root> | --scan <root>] [--depth N] [--dry-run]
  xm project gc [--dry-run]
  xm project register [<cwd>] [--quiet]      (internal, used by dispatcher)
`);
  return 0;
}
