...
이 리포트의 결론이 기본값에 반영되었다: `MODEL_PROFILES.default.executor`가 `opus` → **`sonnet`**으로 변경됨.
본문의 "기본 프로필(default)에서는 executor도 opus다"라는 각주는 이제 사실이 아니다 — 스윗스팟(설계 상위 모델 + sonnet 실행)이 기본값이 되었고, 설계 역할들은 inherit(세션 모델)로 라우팅된다.
large 태스크는 deep-executor(=inherit)로 승격되므로 무거운 실행은 자동으로 상위 모델을 탄다.

같은 날 후속: 실험 3/3b의 결론("§7.5 열거가 sonnet 실행의 안전조건")에 맞춰 `plan-check`의
`failure-mode-coverage`가 **모델 인지형 게이트**로 격상되었다 — 위험 도메인 태스크가 stress
done_criteria 없이 sonnet 이하 모델로 실행될 예정이면 error(`passed:false`, Plan 게이트 차단),
opus/inherit면 warn 유지(확률적 쿠션), done_criteria에 `none — <근거>` 명기 시 통과(밸브).
