...
xm은 Claude Code 마켓플레이스 플러그인으로 배포되지만, 16개 SKILL을 **Cursor**, **OpenAI Codex CLI**, **AWS Kiro**, **Google Antigravity**, **OpenCode**가 인식하는 형식으로도 변환할 수 있습니다. 단일 소스 컴파일러(`xm/lib/install/install-cli.mjs`)가 도구별 산출물을 생성하므로 소스를 한 곳에서만 유지하면 됩니다.
...
| 기능 | Claude Code | Cursor | Codex CLI | Kiro | Antigravity | OpenCode |
|------|:-:|:-:|:-:|:-:|:-:|:-:|
| xm SKILL 본문 | ✅ | ✅ `.cursor/rules/*.mdc` | ✅ `.agents/skills/xm-*/SKILL.md` + `plugins/xm/skills/*/SKILL.md` | ✅ `.kiro/steering/*.md` | ✅ `.agent/skills/*.md` (또는 `~/.gemini/antigravity/skills/`) | ✅ `.opencode/skills/*/SKILL.md` (또는 `~/.config/opencode/skills/*/SKILL.md`) |
| 자동 룰 로딩 | ✅ | △ description 기반 agent-requested | ✅ native Plugin Skill discovery | △ inclusion: auto/manual | △ agent-requested | ✅ native skill discovery |
| 명시 호출 디스커버리 | ✅ `/xm:build` | △ `.cursor/commands/` (현재 미정의) | ✅ `$` 자동완성의 `$xm-build`, 직접 호출 `$xm:build` | ❌ | △ workflows 파일 | ❌ skill-only |
| 차단형 훅 (exit 2) | ✅ | ✅ `.cursor/hooks.json` (camelCase) | △ Bash/shell만 안정 (issue #16732) | ❌ run-only, 차단 불가 | ❌ programmable API 없음 | ❌ 생성 안 함 |
| References 분리 파일 | ✅ | ✅ `xm-*-ref-*.mdc` | △ 본문 인라인 | ✅ `#[[file:…]]` include | △ 본문 인라인 | △ 본문 인라인 |
| CLI 번들링 (build/solver/memory) | ✅ `${CLAUDE_PLUGIN_ROOT}` | ✅ `~/.cursor/xm/lib/` | ✅ `~/.codex/xm/lib/` | ✅ `~/.kiro/xm/lib/` | ✅ `~/.gemini/xm/lib/` | ✅ `~/.config/opencode/xm/lib/` |
| MCP 서버 | ✅ | ✅ | ✅ | ✅ | ✅ (config 전용) | ✅ |
...
> 동일한 SKILL 소스가 지원되는 모든 도구로 렌더링됩니다. `xm install` 실행 전에 각 도구 IDE를 깔아둘 필요는 없습니다 — 다음에 해당 도구로 프로젝트를 열면 룰이 인식됩니다.
...
대화형 선택기는 `local`/`global`을 먼저 묻고, 도구는 번호, 이름, `all`,
또는 `open` → `opencode` 같은 고유 fuzzy 조각으로 선택할 수 있습니다.
...
```bash
# 로컬 — project marketplace를 한 번 등록
node xm/lib/install/install-cli.mjs --target codex --local
codex plugin marketplace add "$PWD"
codex plugin add xm@<installer가-출력한-marketplace>
...
생성:
- `~/.agents/skills/xm-<skill>/SKILL.md` (`--local`이면 `./.agents/skills/xm-<skill>/SKILL.md`) — `$xm-op` 같은 검색 가능한 standalone alias
...
2. `$xm-op refine 초안`으로 검색 가능한 standalone alias를 사용하거나 `$xm:build phased rollout 계획`으로 Plugin Skill을 직접 호출. `$` 자동완성에는 standalone `$xm-*` alias가 표시됨.
...
생성:
- `.opencode/skills/xm-<plug>/SKILL.md` × 16 (프로젝트) 또는 `~/.config/opencode/skills/xm-<plug>/SKILL.md` (`--global`) — `name`, `description`, `compatibility: opencode` frontmatter를 가진 native OpenCode skill
- `.opencode/xm/manifest.json` (프로젝트) 또는 `~/.config/opencode/xm/manifest.json` (`--global`)

OpenCode에서 검증:
1. 프로젝트 루트에서 `opencode` 실행.
2. "use xm build to plan a phased rollout"처럼 xm workflow를 요청. OpenCode가 매칭되는 skill을 발견해야 함.
3. 글로벌 설치는 `~/.config/opencode/skills/xm-build/SKILL.md` 존재 확인. 이미 세션이 떠 있었다면 OpenCode 재시작.

주의:
- OpenCode는 일부 Claude-compatible skill 위치도 읽지만, xm은 Claude compatibility를 꺼도 유지되도록 native `.opencode` / `~/.config/opencode` 경로에 설치합니다.
- OpenCode용 훅은 생성하지 않습니다. 변환된 hook 동작이 필요하면 Cursor 또는 Codex를 사용하세요.
...
# 현재 카운트는 `bun test test/install.test.mjs`로 확인
```

검증 범위: 입력 검증, `--list`/`--dry-run` (부작용 0), 지원되는 모든 도구 install + 멱등성, 공급망 checksum 가드 (정상 + bypass 둘 다), `--verify` (clean / 변조 / 누락), 제거 (사용자 콘텐츠 + 외부 파일 보존), 파일 권한 (`0o644` local, `0o600` global).
...
최근 실행 결과는 `bash xm/scripts/test-install.sh`로 확인하세요.
...
| Codex CLI | Plugin 설치 후 새 thread에서 `$xm:build` 입력 → `$` 자동완성과 Skill 실행 확인 |
...
| Codex `$xm:build`가 표시되지 않음 | Plugin을 설치/갱신하지 않았거나 설치 전 thread를 계속 사용 중 | `xm install --target codex --global` 재실행 후 출력된 `codex plugin add xm@<marketplace>` 실행, 새 thread 시작 |
...
`--verify`가 `selfChecksum: FAIL`이면 manifest 자체가 변조됨. install 재실행으로 재생성하고, 도구별 `xm/manifest.json`을 installer 외부에서 수정한 프로세스가 무엇인지 점검 권장.
