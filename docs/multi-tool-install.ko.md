...
|------|:-:|:-:|:-:|:-:|:-:|:-:|
| 16 SKILL 본문 | ✅ | ✅ `.cursor/rules/*.mdc` | ✅ `.codex/prompts/*.md` (또는 `--global` 시 `~/.codex/prompts/`) | ✅ `.kiro/steering/*.md` | ✅ `.agent/skills/*.md` (또는 `~/.gemini/antigravity/skills/`) | ✅ `.opencode/skills/*/SKILL.md` (또는 `~/.config/opencode/skills/*/SKILL.md`) |
| 자동 룰 로딩 | ✅ | △ description 기반 agent-requested | ❌ 사용자 호출 `/prompts:xm-<plug>` | △ inclusion: auto/manual | △ agent-requested | ✅ native skill discovery |
| 슬래시 명령 디스커버리 | ✅ `/xm:build` | △ `.cursor/commands/` (현재 미정의) | △ `/prompts:xm-build` | ❌ | △ workflows 파일 | ❌ skill-only |
