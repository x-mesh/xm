...
3. **Fallback (only if step 2 fails — corrupted cache, edge case):** resolve the path via Bash `ls -d ~/.claude/plugins/cache/x-kit/x-kit/*/skills/kit/ | sort -V | tail -1` — `sort -V` on the full path picks the semver-latest version (so `1.31.10` beats `1.9.0`). Use that path as your base.
