...
```bash
node .codex/xm/lib/x-build-cli.mjs mode show 2>/dev/null | head -1
```
...
All commands via:
```bash
node .codex/xm/lib/x-build-cli.mjs <command> [args]
```

Shorthand in this document: `$XMB` = `node .codex/xm/lib/x-build-cli.mjs`

> **⚠ When using Bash tool, define the helper once at session start. `.codex/xm` is substituted in SKILL.md prompt text but NOT as a Bash env var — relying on it alone causes `Cannot find module '/lib/...'` errors.**
> ```bash
...
>   local cli=""
>   for cand in \
>     ".codex/xm:-}/lib/server/xm-client.mjs" \
>     ".codex/xm:-}/lib/x-build-cli.mjs" \
