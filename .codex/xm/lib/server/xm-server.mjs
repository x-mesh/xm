#!/usr/bin/env bun
/**
 * xm-server.mjs — Persistent HTTP server for xm CLI tools
 *
 ...
 *
 * Usage: bun xm-server.mjs [--port N] [--idle-timeout N]
 */
...
function resetIdleTimer() {
  ...
  idleTimer = setTimeout(() => {
    console.log(`[xm-server] Idle for ${IDLE_TIMEOUT_MS / 1000}s, shutting down.`);
    shutdown();
  }, IDLE_TIMEOUT_MS);
}
...
function shutdown() {
  console.log('[xm-server] Shutting down...');
  removePIDFile();
  server.stop();
  process.exit(0);
}
...
writePIDFile();
resetIdleTimer();
console.log(`[xm-server] Started on http://127.0.0.1:${PORT} (pid: ${process.pid})`);
console.log(`[xm-server] Idle timeout: ${IDLE_TIMEOUT_MS / 1000}s`);
console.log(`[xm-server] PID file: ${PID_FILE}`);
